function [PTS_ROT,PTS_RO,PTS_AG,ELE_AG,KPTS_ROT] = get_rotor_moved_meshed_edited_dyn_mod(del_t,OME_MECH,PTS_ROT,PTS_RO,PTS_SI,...
    npts_ro,npts_si,pts_ofset,KPTS_ROT,THIC_AIRGAP,ecc,ti,OME_MECH_2) 

% PTS_RO(:,3) = PTS_RO(:,3) + THIC_AIRGAP*ecc;

TH    = OME_MECH * (ti-1)*del_t;

TH_2  = OME_MECH_2 * (ti-1)*del_t;

xd = THIC_AIRGAP*ecc*cos(TH_2);
% yd = THIC_AIRGAP*ecc*sin(TH_2);
...
%%% Need to confirm from SDG ... 22.04.2020
PTS_ROT(:, 3) = PTS_ROT(:, 3) ;
KPTS_ROT(:,3) = KPTS_ROT(:,3) ;
