function [PTS_ROT,PTS_RO,PTS_AGM,ELE_AGM,KPTS_ROT] = get_rotor_moved_meshed_edited_dyn(del_t,OME_MECH,PTS_ROT,PTS_RO,PTS_SI,...
    npts_ro,npts_si,pts_ofset,KPTS_ROT,THIC_AIRGAP,ecc,ti)

% PTS_RO(:,3) = PTS_RO(:,3) + THIC_AIRGAP*ecc;

TH  = OME_MECH * (ti-1)*del_t;
xd = THIC_AIRGAP*0.5*ecc*cos(TH)+THIC_AIRGAP*0.5*ecc;
yd = THIC_AIRGAP*0.5*ecc*sin(TH);
